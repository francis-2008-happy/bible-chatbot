document.addEventListener('DOMContentLoaded', function () {
    ...
    // Modal functionality
    aboutButton.addEventListener('click', () => modal.style.display = "block");
    closeModal.addEventListener('click', () => modal.style.display = "none");
    window.addEventListener('click', (event) => {
        if (event.target === modal) modal.style.display = "none";
    });

    // Main function to send question to backend
    ...
    function addMessage(text, sender, botData = {}) {
        ...
        if (sender === 'assistant') {
            ...
            botName.textContent = botData.bot_name || 'Assistant';
    ...
    function showReferences(reviews) {
        ...
        if (!reviews || reviews.length === 0) {
            referencesContainer.innerHTML = '<p class="no-references">No references found</p>';
            return;
        }

        const referencesTitle = document.createElement('h3');
        referencesTitle.textContent = 'Relevant References';
        ...
        reviews.forEach((review, index) => {
            ...
            if (typeof review === 'object') {
                title = review.metadata?.title || `Reference ${index + 1}`;
                content = review.page_content || JSON.stringify(review);
            } else {
